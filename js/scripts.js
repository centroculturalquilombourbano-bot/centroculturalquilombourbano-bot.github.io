...
const Utils = {
  ...
  smoothScrollTo(element, duration = 1000) {
    ...
    const targetPosition = Math.max(elementPosition - headerHeight - 20, 0);
    const startPosition = window.pageYOffset;
    const distance = targetPosition - startPosition;
    let startTime = null;
    const ease = typeof this.easeInOutQuad === 'function'
...
class App {
  ...
  handleInitialHash() {
    const scrollToHash = (hash) => {
      ...
      if (destination) {
        Utils.smoothScrollTo(destination, 700);
      }
    };

    const initialHash = window.location.hash;
    if (initialHash) {
      setTimeout(() => scrollToHash(initialHash), 200);
    }
